  ...
  test("registers every visible module field and result action", () => {
    ...
    for (const module of modules) {
      for (const field of module.fields)
        expect(
          uiFeatureRegistry.some(
            (entry) => entry.id === `${module.id}:field:${field.id}` || module.id === "video-remix",
          ),
        ).toBeTrue();
      for (const action of module.result.actions)
        expect(uiFeatureRegistry.some((entry) => entry.moduleId === module.id && entry.action === action)).toBeTrue();
    }
  });
});
