...
describe("Seedance model integration",()=>{
  test("has exactly the three approved models and Fast as the only default",()=>{
    ...
    expect(auditSdkRegistry().filter(item=>item.capability==="video-generate").map(item=>item.model)).toEqual([...seedanceModelIds]);
  ...
  test("retires nonterminal Wan jobs without rewriting terminal history",()=>{
    const dir=mkdtempSync(resolve(tmpdir(),"yaozuo-seedance-test-"));tempDirs.push(dir);const path=resolve(dir,"jobs.sqlite");
    const first=new SqliteJobStore(path);const now=new Date().toISOString();
    const base:JobRecord={id:crypto.randomUUID(),ownerUserId:"owner",moduleId:"video-remix",title:"legacy",status:"queued",progress:20,stage:"画面生成",overallExecutionMode:"real",values:{},executionPlan:[{id:"wan",capability:"video-generate",executionMode:"real",implementation:"aihubmix-video",provider:"aihubmix",model:"wan2.6-t2v",startedAt:now}],provenance:[],cancelRequested:false,stagingKeys:[],jobSchemaVersion:1,createdAt:now,updatedAt:now};
    first.create(base);first.create({...base,id:crypto.randomUUID(),status:"succeeded",progress:100,stage:"已完成"});first.db.close();
    const migrated=new SqliteJobStore(path);const jobs=migrated.db.query("SELECT status,error_json FROM jobs ORDER BY created_at,id").all() as Array<{status:string;error_json:string|null}>;
    expect(jobs.some(job=>job.status==="failed"&&job.error_json?.includes("MODEL_RETIRED"))).toBe(true);
    expect(jobs.some(job=>job.status==="succeeded")).toBe(true);migrated.db.close();
  });
});
