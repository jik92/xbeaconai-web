...
describe("AI creation capability contract",()=>{
  test("uses the product-specific defaults without exposing unintegrated video models",()=>{
    ...
    expect(video.every(item=>item.maxOutputs===1&&!item.supportsSeed&&item.referenceModes.join(",")==="omni")).toBeTrue();
  ...
  test("rejects unsupported paid parameters before queueing",()=>{
    const valid={creationKind:"video",type:"视频",prompt:"测试",modelId:"doubao-seedance-2-0-260128",ratio:"9:16",resolution:"720p",count:"1",referenceMode:"omni",duration:"5",seed:""};
