...
import { app, accounts, queue, store } from "../../server/app";
...
const allClosedConfig:PublicAppConfig={
  ...
  menuFeatures:{
    ...
    aiToolbox:{"ai-generate":false,"video-cut":false,"media-understand":false,"video-mashup":false,"voice-clone":false,"video-renewal":false,"subtitle-erase":false,"video-enhancement":false,"kickart":false},
...
describe("unified project config",()=>{
  test("covers all menu entries and keeps the approved defaults",()=>{
    const configuredModuleIds=[...Object.keys(APP_CONFIG.menuFeatures.creationWorkflow),...Object.keys(APP_CONFIG.menuFeatures.aiToolbox)].sort();
  ...
  test("boolean fixtures control every group and produce a loop-free fallback",()=>{
    ...
    const portraitOnly:PublicAppConfig={...allClosedConfig,menuFeatures:{...allClosedConfig.menuFeatures,assets:{portraits:true}}};
    expect(resolveHomeDestination(modules,portraitOnly)).toEqual({kind:"route",path:"/assets/portraits"});

    const remixOnly:PublicAppConfig={...allClosedConfig,menuFeatures:{...allClosedConfig.menuFeatures,creationWorkflow:{...allClosedConfig.menuFeatures.creationWorkflow,"video-remix":true}}};
    expect(resolveHomeDestination(modules,remixOnly)).toEqual({kind:"route",path:"/aigc/video-remix"});

    const toolboxOnly:PublicAppConfig={...allClosedConfig,menuFeatures:{...allClosedConfig.menuFeatures,aiToolbox:{...allClosedConfig.menuFeatures.aiToolbox,"video-cut":true}}};
  ...
  test("Hono rejects disabled modules before any persistent side effect",async()=>{
    const email=`project-config-${crypto.randomUUID()}@example.com`,password="Workflow12345";
    const registration=await app.request("http://local/api/auth/register",{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({email,password,displayName:"工作流验收"})});
    ...
    const jobsBefore=store.db.query("SELECT COUNT(*) AS count FROM jobs WHERE owner_user_id=?").get(auth.user.id) as {count:number};

    for(const moduleId of Object.keys(APP_CONFIG.menuFeatures.creationWorkflow)){
      const idempotencyKey=crypto.randomUUID();
      const response=await app.request(`http://local/api/${moduleId}/jobs`,{method:"POST",headers:{"content-type":"application/json",authorization:`Bearer ${auth.token}`,"idempotency-key":idempotencyKey},body:JSON.stringify({title:"blocked",values:{},allowMockFallback:true})});
      expect(response.status).toBe(403);
      expect((await response.json() as {error:{code:string}}).error.code).toBe("FEATURE_NOT_OPEN");
      expect(store.db.query("SELECT COUNT(*) AS count FROM jobs WHERE owner_user_id=? AND idempotency_key=?").get(auth.user.id,idempotencyKey)).toEqual({count:0});
    }

    const jobsAfter=store.db.query("SELECT COUNT(*) AS count FROM jobs WHERE owner_user_id=?").get(auth.user.id) as {count:number};
    const meResponse=await app.request("http://local/api/auth/me",{headers:{authorization:`Bearer ${auth.token}`}});
