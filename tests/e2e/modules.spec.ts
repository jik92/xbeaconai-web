...
async function authorization(page: Page) {
  const token = await page.evaluate(() => localStorage.getItem("yaozuo:auth-token:v1"));
...
test.beforeEach(async ({ page, request }, testInfo) => {
  ...
  const response = await request.post("/api/auth/register", {
    data: { email, password: testPassword, displayName: "端测用户" },
  });
...
async function completeField(page: Page, field: (typeof modules)[number]["fields"][number]) {
  ...
  if (["video", "audio", "image"].includes(field.kind)) {
    await page.locator(`#${field.id}`).setInputFiles({
      name: `${field.id}.mp4`,
      mimeType: field.kind === "audio" ? "audio/mpeg" : "video/mp4",
      buffer: Buffer.from("mock"),
    });
  } else if (field.kind === "asset-group") {
    await page
      .locator(`#${field.id}`)
      .setInputFiles({ name: `${field.id}.png`, mimeType: "image/png", buffer: Buffer.from("mock-image") });
...
for (const module of modules) {
  ...
  test(`${module.label} exposes its complete business workflow`, async ({ page }) => {
    ...
    for (const field of module.fields.slice(0, splitAt)) {
      await expect(page.locator(".field > label").filter({ hasText: field.label }).first()).toBeVisible();
      await completeField(page, field);
    }
    await page.getByRole("button", { name: "下一步" }).click();
    for (const field of module.fields.slice(splitAt)) {
      await expect(page.locator(".field > label").filter({ hasText: field.label }).first()).toBeVisible();
      await completeField(page, field);
    }
    await page.getByRole("button", { name: "下一步" }).click();
...
test("closed creation workflows show Coming Soon while AI tools stay open", async ({ page }) => {
  ...
  await expect(page.locator(".ag-composer")).toBeVisible();
...
test.skip("legacy AI creation composer matches the product configuration flow without a paid request", async ({
  page,
}, testInfo) => {
...
test.skip("legacy AI creation image Mock submission completes through the async queue", async ({ page }, testInfo) => {
...
test("AI creation replica completes its browser-only Mock workflow", async ({ page }, testInfo) => {
  test.skip(testInfo.project.name !== "desktop", "AI creation replica is exercised once on desktop");
  await page.goto("/tools/ai-generate");
  const businessRequests: string[] = [];
  page.on("request", (request) => {
    if (/\/api\/(models|uploads|[^/]+\/jobs)|\/openapi\.json/.test(request.url())) businessRequests.push(request.url());
  });
  const composer = page.locator(".ag-composer");
  await expect(composer).toBeVisible();
  expect(Math.round((await composer.boundingBox())!.width)).toBe(900);
  expect(Math.round((await composer.boundingBox())!.height)).toBe(204);
  await expect(page.getByRole("button", { name: /字节Seedance 2.0/ })).toBeVisible();
  await expect(page.getByText("全能参考", { exact: true })).toBeVisible();
  await expect(page.getByRole("button", { name: /9:16.*720P/ })).toBeVisible();
  await expect(page.getByRole("button", { name: /5s.*1个/ })).toBeVisible();
  await expect(page.getByText("提交前手动确认")).toBeVisible();
  await page.getByRole("button", { name: "视频生成", exact: true }).click();
  await page.getByRole("button", { name: "图片生成", exact: true }).click();
  await expect(page.getByRole("button", { name: /字节Seedream 4.0/ })).toBeVisible();
  await page.getByRole("button", { name: "图片生成", exact: true }).click();
  await page.getByRole("button", { name: "视频生成", exact: true }).last().click();
  await page.getByRole("button", { name: /字节Seedance 2.0/ }).click();
  await expect(page.getByRole("dialog", { name: "选择生成模型" })).toBeVisible();
  await expect(page.getByText("字节Seedance 2.0 Mini")).toBeVisible();
  await page.getByText("字节Seedance 2.0 Fast").click();
  await page.getByRole("button", { name: /9:16/ }).click();
  await page.getByRole("button", { name: "16:9" }).click();
  await page.getByRole("button", { name: /5s.*1个/ }).click();
  await page.getByRole("button", { name: "2个" }).click();
  await page.getByRole("button", { name: "10s" }).click();
  await page.getByLabel("创作指令").fill("一只猫在雨夜穿过霓虹街道");
  await page.getByText("提交前手动确认").click();
  await page.getByRole("button", { name: "提交生成" }).click();
  await expect(page.getByRole("dialog", { name: "确认提交生成" })).toBeVisible();
  await page.getByRole("button", { name: "确认提交 Mock" }).click();
  await expect(page.getByText("Mock 作品")).toBeVisible({ timeout: 3000 });
  await page.getByRole("button", { name: "查看结果" }).click();
  await expect(page.getByRole("dialog", { name: "Mock 结果预览" })).toBeVisible();
  await page.getByRole("button", { name: "关闭" }).click();
  await page.getByRole("button", { name: "继续追问" }).click();
  await expect(page.getByLabel("创作指令")).toHaveValue(/继续优化/);
...
test("navigation keeps all twelve modules reachable", async ({ page }) => {
  await page.goto("/tools/ai-generate");
  for (const module of modules)
    if (isModuleOpen(module.id)) await expect(page.getByRole("link", { name: new RegExp(module.label) })).toBeVisible();
    else await expect(page.getByLabel(`${module.label} Coming Soon`)).toBeVisible();
});
...
test("account workspace supports registration, settings, recharge and password lifecycle", async ({
  page,
}, testInfo) => {
  ...
  await expect(page.getByRole("heading", { name: "视频分割", exact: true })).toBeVisible();
...
test("active tasks can be cancelled and retried", async ({ page }, testInfo) => {
  ...
  const [response] = await Promise.all([
    page.waitForResponse((item) => item.url().includes("/api/ad-script/jobs") && item.request().method() === "POST"),
    page.getByRole("button", { name: "生成 3 版脚本" }).click(),
  ]);
  ...
  await expect
    .poll(async () =>
      ((await page.request.get(`/api/jobs/${created.id}`, { headers })).json() as Promise<{ status: string }>).then(
        (item) => item.status,
      ),
    )
    .toBe("cancelled");
...
test("every generic result action is executable end to end", async ({ page, context }, testInfo) => {
  ...
  await page.goto("/tools/ai-generate");
  const headers = await authorization(page);
  const generic = modules.filter(
    (module) => module.id !== "video-remix" && module.id !== "ai-generate" && isModuleOpen(module.id),
  );
  const jobs = await Promise.all(
    generic.map(async (module) => {
      const title = `action-matrix-${module.id}-${Date.now()}`;
      const response = await page.request.post(`/api/${module.id}/jobs`, {
        headers: { ...headers, "Idempotency-Key": crypto.randomUUID() },
        data: { title, values: { type: "图片" }, allowMockFallback: true },
      });
  ...
  await Promise.all(
    jobs.map(async (job) => {
      for (let attempt = 0; attempt < 100; attempt += 1) {
        const response = await page.request.get(`/api/jobs/${job.id}`, { headers });
        const body = (await response.json()) as { status: string };
        if (["succeeded", "partially_succeeded"].includes(body.status)) return;
        if (["failed", "cancelled"].includes(body.status)) throw new Error(`${job.module.id} ended as ${body.status}`);
        await new Promise((resolve) => setTimeout(resolve, 250));
      }
      throw new Error(`${job.module.id} timed out`);
    }),
  );
  for (const job of jobs) {
    ...
    const open = async () => {
      await row.getByRole("button", { name: "查看结果" }).click();
    ...
    for (const action of job.module.result.actions) {
      ...
      await button.click();
      if (action.includes("一键成片") || action.includes("用于混剪") || action.includes("合并片段")) {
        await expect(page).not.toHaveURL(new RegExp(`${job.module.path.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}$`));
        await page.goto(job.module.path);
      } else if (action.includes("删除") || /再次|重新|调整|改写|变体|替换|编辑|继续/.test(action))
        await expect(page.locator(".result-drawer")).toBeHidden();
...
test("every specialized remix control is actionable", async ({ page, context }, testInfo) => {
  ...
  await page
    .locator("#remix-source")
    .setInputFiles({ name: "remix-controls.mp4", mimeType: "video/mp4", buffer: Buffer.from("mock") });
  ...
  const refreshed = page.waitForResponse(
    (response) => response.url().includes("/api/video-remix/jobs") && response.request().method() === "POST",
  );
  ...
  await expect
    .poll(
      async () =>
        (
          (await page.request.get(`/api/jobs/${refreshedJob.id}`, { headers })).json() as Promise<{ status: string }>
