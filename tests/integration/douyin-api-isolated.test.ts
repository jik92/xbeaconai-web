...
// ── Isolated test database process ──────────────────────────────────
...
describe("douyin API integration (isolated DB)", () => {
  ...
  beforeEach(async () => {
    ...
    if (!folder) throw new Error("TEST_DEFAULT_FOLDER_MISSING");
    folderId = folder.id;
    ...
    if (!otherFolder) throw new Error("TEST_OTHER_FOLDER_MISSING");
  ...
  test("GET /api/imports/share-content/{jobId} returns 404 for another user's job", async () => {
    ...
    if (!otherUser) throw new Error("TEST_OTHER_USER_MISSING");
