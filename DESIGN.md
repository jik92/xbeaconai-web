...
**`save-location-picker`** — Shared compact save-folder control for task creation, media import, and video export. Use the native Select appearance, preserve folder hierarchy and the default marker, and provide one restrained inline “新建文件夹” action. A newly created root folder becomes selected and is requested as the account default; creation/default failures remain visible beside the control without closing the current dialog. Pages own only the selected folder ID and must not duplicate folder queries, sorting, initialization, or creation behavior.
