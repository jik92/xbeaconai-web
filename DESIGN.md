...
**`sidebar-navigation`** — First-level groups are collapsed by default and use compact ghost triggers to reveal their menu items. Keep the navigation content aligned near the top and place the vertical scrollbar against the drawer edge. Active items retain the same typography size as inactive items and use only the restrained surface state for emphasis. The global icon-only drawer mode continues to show every visible item regardless of group expansion.
...
**`workflow-header`** — Multi-stage creation modules use one persistent 56px Header across configuration, processing, and result states, separated from the workspace by one subtle `shadow-sm`. The left identifies the module, the center is a non-interactive semantic `ol` with numbered current/future stages and checked completed stages, and the right contains `生成记录` as `outline + sm` plus `新建` as `ghost + sm`; module-specific primary or save actions may precede them. Video remix, one-click video creation, and ad-script share this structure while keeping module-specific stage labels. Do not duplicate the workflow as clickable content tabs or render separate page-title bars in later states; navigation remains in the page content. At constrained widths hide stage and action labels before removing status circles or accessible labels.

**`dashed-picker-tile`** — Resource picker triggers use one shared shadcn Button composition with a dashed hairline, compact radius, restrained hover/focus feedback, and semantic 12px labels. Use the 64px × 80px compact presentation beside media thumbnails and the full-width 64px-high presentation for sidebar fields. Empty and selected triggers share the same interaction shell; previews, names, and descriptions may change, but feature CSS must not override the component's border, radius, typography, hover, focus, or disabled states.

**`attachment-picker`** — The shared attachment dialog uses structural hairlines only around the dialog workspace, search field, and preview columns. Source tabs, folder-tree rows, breadcrumbs, folders, and asset cards use `ghost + sm` without individual borders; selected rows use the restrained strong surface plus the existing check mark. Footer cancel uses `outline + sm`, confirmation uses `default + sm`, and close actions use `ghost + icon-sm`. Attachment CSS owns geometry, spacing, scrolling, truncation, and selected layout state only; shared Button variants own borders, radius, typography, hover, focus, and disabled states.

**`segmented-control`** — Two or more mutually exclusive compact modes use one shared segmented control: a `surface-strong` track with 8px radius and 4px padding, containing `ghost + sm` Buttons. The selected segment uses the normal surface and one subtle shadow. Labels must name every available mode directly; do not substitute a binary Switch when users need to compare named modes. The ad-script scene category and video-remix creation mode share this component.
...
**`portrait-picker`** — One shared compact portrait dialog is used by video remix and one-click video creation. It owns the source, gender and text filters, complete portrait cards, temporary selection, selection limit feedback, loading/error/empty states, incremental loading and confirmation footer. Single-select and bounded multi-select use the same interaction; pages provide only `maxSelect`, current keys and confirmed-state adapters. Cancelling never mutates page state. Do not build module-specific portrait modals or CSS.
...
**`provider-disabled`** — Features that depend on an unverified Provider remain visible but use the existing muted disabled treatment and cannot be opened or submitted. Direct routes show one compact neutral state with the failed Provider reason. Static `menuFeatures: false` entries keep their separate hidden/Coming Soon behavior.
...
**`save-location-picker`** — Shared compact save-folder control for task creation, media import, and video export. Every AI 工具箱 creation form shows this control and initially defaults to “不保存素材库”. Selecting a folder enables library saving and persists that account-synced module default, so changing one tool never changes another tool's destination; selecting “不保存素材库” clears it. Use the native Select appearance, preserve folder hierarchy, distinguish the current task default from the account-wide material default, and provide one restrained inline “新建文件夹” action. A newly created root folder becomes selected and becomes only the current tool's default; creation/default failures remain visible beside the control without closing the current dialog. Pages own only the selected folder ID and must not duplicate folder queries, sorting, initialization, or creation behavior.
