...
**`save-location-picker`** — Shared compact save-folder control for task creation, media import, and video export. Every AI 工具箱 creation form shows this control and persists its own account-synced module default, so changing one tool never changes another tool's destination. Use the native Select appearance, preserve folder hierarchy, distinguish the current task default from the account-wide material default, and provide one restrained inline “新建文件夹” action. A newly created root folder becomes selected and becomes only the current tool's default; creation/default failures remain visible beside the control without closing the current dialog. Pages own only the selected folder ID and must not duplicate folder queries, sorting, initialization, or creation behavior.
