...
import { isSeedanceModelId, type SeedanceModelId, type SeedanceReferenceKind } from "../server/models/video-models";
...
/** Executes one persisted job. Queue transport and concurrency live in the BullMQ worker. */
export class JobProcessor {
  ...
  private async runSeedance(job: JobRecord, model: SeedanceModelId) {
    ...
    if (!taskId) {
      ...
      let references: Awaited<ReturnType<JobProcessor["prepareSeedanceReferences"]>>;
      try {
  ...
  private async runVideoCut(job: JobRecord) {
    if (!this.accounts) throw new Error("素材所有权服务不可用");
    if (!ossutils.configured) throw new Error("TOS_NOT_CONFIGURED: 视频切片必须保存到素材文件夹的 TOS 目录");
    const sourceAssetId = job.values.source?.split(":", 3)[1];
    if (!sourceAssetId) throw new Error("视频素材标识无效");
    const sourceAsset = this.accounts.getOwnedAsset(job.ownerUserId, sourceAssetId);
    if (!sourceAsset?.mimeType.startsWith("video/")) throw new Error("视频素材不存在或不属于当前账号");
    const folderId = job.values.outputFolderId;
    const folder = folderId ? this.accounts.getAssetFolder(job.ownerUserId, folderId) : undefined;
    ...
    try {
      ...
      this.change(job.id, { stage: "正在分割视频", progress: 20, provenance: [...plan] });
      ...
      for (const [index, clipPath] of clipPaths.entries()) {
        ...
        if (!latest || latest.cancelRequested) {
          this.change(job.id, { status: "cancelled", stage: "已取消" });
          return;
        }
      ...
      if (this.accounts.taskNotificationsEnabled(job.ownerUserId))
  ...
  private async runVideoClipMerge(job: JobRecord) {
    if (!this.accounts) throw new Error("素材所有权服务不可用");
    ...
    const assets = assetIds.map((id) => this.accounts?.getOwnedAsset(job.ownerUserId, id));
    ...
    try {
      ...
      this.change(job.id, { stage: "正在按原始切片顺序合并", progress: 45 });
      ...
      const assetId = crypto.randomUUID();
      const storageKey = `${folder.storagePrefix}generated/${job.id}/${fileName}`;
      this.change(job.id, { stage: "正在保存合并视频", progress: 80 });
      await ossutils.putLibraryFile({
        filePath: outputPath,
        key: storageKey,
        mimeType: "video/mp4",
  ...
  private async run(id: string) {
    ...
    if (job.moduleId === "video-cut" && job.values.mergeMode === "video-cut-clips") {
      await this.runVideoClipMerge(job);
      return;
    }
    if (job.moduleId === "video-cut") {
      await this.runVideoCut(job);
      return;
    }
