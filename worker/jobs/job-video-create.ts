...
async function saveShotAudio(job: JobRecord, context: JobHandlerContext, shotId: string, text: string) {
  ...
  const name = `${job.id}-${shotId}-voice.mp3`;
  const output = resolve(env.dataDir, "results", name);
  const response = await volcSpeech.synthesize({
  ...
  async execute(job, context) {
    ...
    try {
      if (operation === "analyze") {
        ...
        const shouldGenerateAudio = !generatedWithAudio && providerCredentials.isProviderVerified("volc-speech");
        const audioStage = shouldGenerateAudio
          ? stage(job, "speech-synthesis", "real", "volc-tts-v3-unidirectional", env.volcSpeech.presetTtsResourceId)
          : undefined;
        const audio = shouldGenerateAudio ? await saveShotAudio(job, context, shot.id, narration) : undefined;
        ...
        const sourceBytes = response.bytes;
        const sourceExecutionMode = response.executionMode;
