...
export function parseScriptRemixNextAnalysis(text: string): ScriptRemixNextShot[] {
  ...
  return normalizeScriptRemixNextShots(
    parsed.shots.map((shot, index) => ({ ...shot, id: crypto.randomUUID(), ordinal: index + 1 })),
  );
}
...
输出格式：
{"shots":[{"title":"分镜标题","speech":"该镜头对应的完整原文","visual":"画面主体、场景和构图","action":"人物和商品的连续动作","camera":"景别、机位和运镜","durationSeconds":8,"productRequirement":"商品一致性要求","characterRequirement":"人物一致性要求"}]}
...
export function buildStoryboardGridPrompt(input: {
  ...
  const cells = Array.from({ length: 9 }, (_, index) => {
    ...
    return shot
      ? `${index + 1}. ${shot.title}：${shot.visual}；动作：${shot.action}；镜头：${shot.camera}`
...
商品：${input.productName}
人物：${input.portraitName || "按脚本设定"}
画面：${input.shot.visual}
动作：${input.shot.action}
镜头：${input.shot.camera}
商品一致性：${input.shot.productRequirement}
人物一致性：${input.shot.characterRequirement}`;
}
