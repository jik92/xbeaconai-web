...
import { extractAudio, extractFrame, generateSampleVideo, probeMedia } from "../media/ffmpeg";
...
type ArtifactDraft = {id:string;name:string;mimeType:string;url?:string;text?:string;executionMode:JobRecord["overallExecutionMode"]};
...
function assetIdsFromValues(values: Record<string, string>) {
  ...
  for (const value of Object.values(values)) {
    ...
    } catch { /* request validation reports malformed values separately */ }
  }
  return [...ids];
}
...
function outputKind(job: JobRecord): "video" | "audio" | "image" | "text" {
  ...
  if (job.moduleId === "ai-generate") return job.values.type === "图片" ? "image" : job.values.type === "视频" ? "video" : "text";
...
function overallMode(provenance: StageProvenance[], artifacts: ArtifactDraft[]): JobRecord["overallExecutionMode"] {
  const modes = new Set([...provenance.map((stage) => stage.executionMode), ...artifacts.map((artifact) => artifact.executionMode)]);
  return modes.size === 1 ? [...modes][0] : "mixed";
}

export const stageMap: Record<ModuleId, Array<[string, string]>> = {
  "video-remix": [["media-probe","素材校验"],["video-understand","AI 解析"],["text-rewrite","提示词改写"],["storyboard","分镜生成"],["video-generate","画面生成"],["media-compose","合并成片"]],
  "video-create": [["text-generate","脚本生成"],["asset-match","素材匹配"],["speech-synthesize","智能配音"],["subtitle-align","字幕对齐"],["media-compose","智能成片"]],
  "ad-script": [["text-generate","脚本生成"],["structured-output","结构校验"]],
  "ai-generate": [["prompt-understand","理解指令"],["multimodal-generate","生成内容"]],
  "video-cut": [["media-probe","媒体探测"],["video-split","智能切分"]],
  "media-understand": [["media-probe","媒体探测"],["media-understand","内容理解"],["timeline-label","时间轴标签"]],
  "video-mashup": [["media-probe","素材校验"],["asset-arrange","素材编排"],["batch-render","批量渲染"]],
  "voice-clone": [["audio-validate","样本验证"],["voice-clone","音色训练"],["speech-synthesize","试听生成"]],
  "video-renewal": [["issue-detect","问题检测"],["video-restore","视频修复"]],
  "subtitle-erase": [["region-track","区域跟踪"],["region-inpaint","擦除补全"]],
  "video-enhancement": [["media-probe","画质评估"],["video-enhance","画质增强"]],
  "kickart": [["variant-plan","裂变规划"],["text-variants","文案变体"],["batch-render","批量渲染"]],
};

const textCapabilities = new Set(["text-generate","structured-output","text-rewrite","storyboard","prompt-understand","text-variants","variant-plan","asset-arrange"]);
const verifiedSdkIds = new Set<string>();
for (const file of ["capabilities.json","ffmpeg-capabilities.json"]) try {
  const capabilityFile = JSON.parse(readFileSync(resolve(env.dataDir,file),"utf8")) as {entries?:Array<{id:string;status:string}>};
  for(const entry of capabilityFile.entries??[])if(entry.status==="verified"||entry.status==="local")verifiedSdkIds.add(entry.id);
} catch { /* no verified capabilities in this environment */ }

export function buildExecutionPlan(moduleId: ModuleId, values: Record<string,string>, videoModel?: SeedanceModelId): StageProvenance[] {
  return stageMap[moduleId].map(([capability], index) => {
    let executionMode: StageProvenance["executionMode"] = "mock", implementation = "yaozuo-mock-provider", model: string | undefined;
    const localSdkId = capability.includes("probe") ? "ffmpeg-probe" : capability.includes("compose") ? "ffmpeg-compose" : capability.includes("split") ? "ffmpeg-split" : capability === "video-denoise" ? "ffmpeg-denoise" : undefined;
    if (localSdkId && verifiedSdkIds.has(localSdkId)) {
      executionMode = "local"; implementation = "ffmpeg-local";
    } else if (!env.forceMock && aihubmix.configured && verifiedSdkIds.has("aihubmix-text") && textCapabilities.has(capability)) {
      executionMode = "real"; implementation = "aihubmix-text"; model = "gpt-4.1-nano-free";
    } else if (!env.forceMock && aihubmix.configured && verifiedSdkIds.has("aihubmix-audio") && capability === "speech-synthesize") {
      executionMode = "real"; implementation = "aihubmix-audio"; model = "tts-1";
    } else if (!env.forceMock && aihubmix.configured && videoModel && capability === "video-generate") {
      executionMode = "real"; implementation = "aihubmix-video"; model = videoModel;
    ...
    return { id:`plan:${index}:${capability}`, capability, executionMode, implementation, provider:executionMode==="real"?"aihubmix":undefined, model, fallbackReason:executionMode==="mock"?"真实接口尚未映射或测试环境强制 Mock":undefined, startedAt:"" };
  });
}
...
export class MemoryJobQueue {
  ...
  start() {
    ...
    for (const job of this.store.recoverable()) {
      ...
      if(job.providerStatus==="submitting"&&!job.providerTaskId){this.store.update(job.id,{status:"failed",stage:"上游提交状态未知",error:{code:"PROVIDER_SUBMISSION_UNKNOWN",message:"上游提交结果未知，需要人工核对以避免重复计费",retryable:false,requestId:crypto.randomUUID()}});continue}
      this.store.update(job.id, { status: "queued", stage: "等待恢复", progress: Math.min(job.progress, 95) });
      this.enqueue(job.id);
    }
  }

  private async recoverObjectCleanup(){for(const item of this.store.pendingObjectCleanup()){try{await ossutils.markCleanupReady(item.object_key);await ossutils.deleteObject(item.object_key);this.store.completeObjectCleanup(item.object_key);const job=this.store.get(item.job_id);if(job)this.store.update(job.id,{stagingKeys:job.stagingKeys.filter(key=>key!==item.object_key)})}catch(error){this.store.deferObjectCleanup(item.object_key,error,item.attempts)}}}
  ...
  private async prepareSeedanceReferences(job: JobRecord) {
    ...
    let totalBytes = 0;
    const prepared: Array<{ kind: SeedanceReferenceKind; path: string; mimeType: string; sizeBytes: number; extension: string }> = [];
    for (const id of ids) {
      ...
      if (!kind) throw new SeedanceFlowError("UNSUPPORTED_REFERENCE_TYPE", `Seedance 不支持素材类型 ${asset.mimeType}`, false);
      counts.set(kind, (counts.get(kind) ?? 0) + 1);
      if ((counts.get(kind) ?? 0) > 1) throw new SeedanceFlowError("TOO_MANY_REFERENCES", `每类最多上传一个${kind}参考`, false);
      ...
      if (!local || local.startsWith("..") || local.startsWith("/") || !existsSync(path)) throw new SeedanceFlowError("INVALID_ASSET_PATH", "素材文件路径无效", false);
      const file = Bun.file(path);
      if (file.size !== asset.byteSize) throw new SeedanceFlowError("ASSET_SIZE_MISMATCH", "素材文件大小与记录不一致", false);
      const probe = await probeMedia(path);
      if (kind === "video" && !probe.streams.some((stream) => stream.codec_type === "video")) throw new SeedanceFlowError("INVALID_VIDEO_REFERENCE", "视频参考无法解码", false);
      if (kind === "audio" && !probe.streams.some((stream) => stream.codec_type === "audio")) throw new SeedanceFlowError("INVALID_AUDIO_REFERENCE", "音频参考无法解码", false);
      if (kind === "image" && !probe.streams.some((stream) => stream.codec_type === "video")) throw new SeedanceFlowError("INVALID_IMAGE_REFERENCE", "图片参考无法解码", false);
      prepared.push({ kind, path, mimeType: asset.mimeType, sizeBytes: asset.byteSize, extension: extname(asset.storageKey) });
    }
    if (prepared.length > 3 || totalBytes > 250 * 1024 * 1024) throw new SeedanceFlowError("REFERENCES_TOO_LARGE", "参考素材总量超过限制", false);
    ...
    for (const item of prepared) {
      ...
      const uploaded = await ossutils.putStagedFile({
        filePath: item.path, sizeBytes: item.sizeBytes, sha256: await sha256File(item.path), mimeType: item.mimeType,
        jobId: job.id, extension: item.extension,
      });
  ...
  private async cleanupStaging(jobId: string) {
    ...
    for (const key of current.stagingKeys) {
      try { await ossutils.markCleanupReady(key); await ossutils.deleteObject(key); }
      catch (error) { failed.push(key); this.store.scheduleObjectCleanup(jobId,key,error); }
    }
  ...
  private async runSeedance(job: JobRecord, model: SeedanceModelId) {
    ...
    if (!taskId) {
      if (job.providerStatus === "submitting") throw new SeedanceFlowError("PROVIDER_SUBMISSION_UNKNOWN", "上游提交结果未知，需要人工核对后再重试", false);
      ...
      try{references=await this.prepareSeedanceReferences(job)}catch(error){await this.cleanupStaging(job.id);throw error}
      this.change(job.id, { providerStatus: "submitting" });
      try {
        const created = await aihubmix.createSeedanceVideo({
          model,
          prompt: job.values.prompt || job.values.topic || job.values.description || "A polished product video in a clean bright studio, stable camera",
          resolution: "720p",
        ...
        this.change(job.id, {
          providerTaskId: taskId, providerStatus: created.status || "submitted", providerSubmittedAt: submittedAt.toISOString(),
        ...
        if (!definitelyRejected) {
          ...
          throw new SeedanceFlowError("PROVIDER_SUBMISSION_UNKNOWN", "上游提交结果未知，需要人工核对以避免重复计费", false);
        }
    ...
    const deadline = Date.parse(this.store.get(job.id)?.providerDeadlineAt ?? new Date(Date.now() + 20 * 60_000).toISOString());
      ...
      while (true) {
        ...
        if (reconciliationReason && !cancelAttempted) {
          ...
          try {
            ...
            this.change(job.id, { providerCancelState: state, providerStatus: "reconciling", stage: reconciliationReason === "cancel" ? "取消核对中" : "超时核对中" });
          } catch {
            this.change(job.id, { providerCancelState: "failed", providerStatus: "reconciling", stage: reconciliationReason === "cancel" ? "取消核对中" : "超时核对中" });
          }
        }
        let task;
        try{task=await aihubmix.getVideo(taskId)}catch{
          this.change(job.id,{providerStatus:"reconciling",stage:reconciliationReason==="cancel"?"取消核对中":reconciliationReason==="timeout"?"超时核对中":"上游状态核对中"});
        ...
        if (["completed", "succeeded"].includes(task.status)) {
          ...
          if (reconciliationReason === "timeout") throw new SeedanceFlowError("UPSTREAM_COMPLETED_AFTER_TIMEOUT", "上游在本地超时后完成，结果已丢弃", true);
  ...
  private async run(id: string) {
    ...
    if (job.values.__scenario === "insufficient-credits") {
      await wait(250);
      this.change(id, { status:"failed", stage:"计费校验", progress:0, error:{code:"INSUFFICIENT_CREDITS",message:"创作点不足，请调整任务或充值",retryable:false,requestId:crypto.randomUUID()} });
      return;
    }
    const stages = stageMap[job.moduleId];
    const plan = job.executionPlan.length ? job.executionPlan : buildExecutionPlan(job.moduleId, job.values, job.videoModel);
    ...
    if (!job) return;
    const provenance: StageProvenance[] = job.providerTaskId ? job.provenance.filter((stage) => Boolean(stage.completedAt)) : [];
    const produced: ArtifactDraft[] = [];
    const resumedVideoIndex = job.providerTaskId ? plan.findIndex((stage) => stage.implementation === "aihubmix-video") : -1;
    for (let index = resumedVideoIndex >= 0 ? resumedVideoIndex : 0; index < stages.length; index += 1) {
      ...
      if (job.values.__scenario === "fail-analysis" && index === Math.min(1, stages.length - 1)) {
        await wait(300);
        this.change(id, { status:"failed", stage:`${label}失败`, progress:42, provenance, error:{code:"PROCESSING_FAILED",message:"素材分析中断，请检查文件后重试",retryable:true,requestId:crypto.randomUUID()} });
        return;
      }
      ...
      try {
        ...
        if(error instanceof SeedanceFlowError&&error.code==="JOB_CANCELLED"){this.change(id,{status:"cancelled",stage:"已取消",error:undefined});return}
        if(stage.implementation==="aihubmix-video"||!env.allowMockFallback){this.change(id,{status:"failed",stage:`${label}失败`,error:{code:error instanceof SeedanceFlowError?error.code:"PROVIDER_ERROR",message:error instanceof Error?error.message:"上游接口失败",retryable:error instanceof SeedanceFlowError?error.retryable:true,requestId:crypto.randomUUID()}});return}
        stage.executionMode="mock";stage.implementation="yaozuo-mock-provider";stage.fallbackReason=error instanceof Error?error.message:"上游接口失败";await wait(350);
      }
    ...
    if (expected !== "text" && !produced.some((artifact) => artifact.mimeType.startsWith(`${expected}/`))) produced.push(await ensureMockMedia(expected));
    const finalArtifacts: ArtifactDraft[] = produced.length ? produced : [{ id: crypto.randomUUID(), name: `${job.moduleId}-result.json`, mimeType: "application/json", text: textSummary[job.moduleId], executionMode: provenance.some((stage) => stage.executionMode === "real") ? "real" : "mock" }];
    const owner=this.accounts?.getUser(job.ownerUserId);
    for(const artifact of finalArtifacts)if(artifact.url&&this.accounts&&owner){this.accounts.createArtifact({id:artifact.id,ownerUserId:job.ownerUserId,jobId:job.id,storageKey:artifact.name,name:artifact.name,mimeType:artifact.mimeType,createdAt:new Date().toISOString()});artifact.url=`/api/artifacts/${artifact.id}`}
    ...
    const result: JobResult = {
      ...
      data: { values: job.values, generatedAt: new Date().toISOString(), mock: artifactMode === "mock" || artifactMode === "mixed" },
    };
    this.change(id, { status: job.values.__scenario === "partial-batch" ? "partially_succeeded" : "succeeded", stage: job.values.__scenario === "partial-batch" ? "部分完成" : "已完成", progress: 100, provenance, result, overallExecutionMode: artifactMode });
    if(owner&&this.accounts?.taskNotificationsEnabled(job.ownerUserId))this.accounts.createNotification(job.ownerUserId,"task_completed","创作任务已完成",`${job.title} 已生成，可前往任务中心查看。`,job.id);
  }
}
