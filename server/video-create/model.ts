import { formatPortraitIdentity, type PortraitGender, parsePortraitTags } from "../../shared/portraits/portrait-tags";
...
export function resolveVideoCreatePortraitIdentity(
  portrait?: string | { name: string; gender?: PortraitGender; description?: string },
) {
  const profile = typeof portrait === "string" ? { name: portrait } : portrait;
  const tags = parsePortraitTags(profile?.name ?? "");
  const gender = profile?.gender ?? tags?.gender;
  const identity = tags
    ? formatPortraitIdentity(tags)
    : [
        gender ? `性别为“${gender}”` : "",
        profile?.description ? `基础描述为${JSON.stringify(profile.description)}` : "",
      ]
        .filter(Boolean)
        .join("，");
...
export async function analyzeVideoCreateProduct(
  assets: MediaAsset[],
  portrait?: { name: string; source_url: string; gender?: PortraitGender; description?: string },
): Promise<VideoCreateRecommendation> {
  const { gender: portraitGender, identity: portraitIdentity } = resolveVideoCreatePortraitIdentity(portrait);
  ...
  const result = await aihubmix.analyzeImages({
    ...
    prompt: `你是中文短视频广告策划。前 ${assets.length} 张是同一商品的商品图片${portrait ? "，最后 1 张是已选出镜人像" : ""}。${portraitIdentity ? `已选出镜人像资料：${portraitIdentity}。${portraitGender ? `presenterGenders 必须使用“${portraitGender === "男" ? "男声" : "女声"}”。` : ""}不得根据商品或目标受众改变已提供的人像资料。` : ""}综合分析图片，只基于可见事实给出可直接生成口播脚本的完整参数。严格返回 JSON：
...
marketingGoals=电商转化/品牌曝光/App下载/门店到店/直播引流；targetAudiences=18-24岁女性/25-35岁女性/18-24岁男性/25-35岁男性/宝妈/学生/职场白领/中老年/全年龄段；presenterRoles=好物推荐员/普通用户/行业专家/品牌官方；presenterGenders=不区分/男声/女声；contentStyles=种草/专业测评/情绪共鸣/悬念叙事/故事/数据说话；openingStyles=自动匹配/痛点直击/数字冲击/福利诱惑/问句互动/品牌声量/随机；closingGuides=硬引导购买/软种草/互动提问；scriptTopics=直播带货/产品功能讲解/痛点解决/对比测评/情感共鸣/节日营销；materialTopics=产品外观/使用体验/价格优势/品质保障/售后服务/用户口碑/生活方式/成分功效/限时优惠；marketingMethods=场景展示/痛点解决/竞品对比/用户证言/专家背书/限时促销；templates=常规/节日营销/明星同款/爆款复制。无法从图片判断时返回空数组或空字符串，不要使用绝对化承诺。`,
  ...
  return portraitGender
    ? { ...recommendation, presenterGenders: [portraitGender === "男" ? "男声" : "女声"] }
    : recommendation;
}
...
export function generateVideoCreateStoryboard(
  ...
  const { gender: portraitGender, identity: portraitIdentity } = resolveVideoCreatePortraitIdentity(
    portrait ?? legacyPortrait?.name,
  );
  const storyboardInput = portraitGender
    ? {
        ...aggregate.project.input,
        presenterGenders: [portraitGender === "男" ? "男声" : "女声"],
      }
    : aggregate.project.input;
...
${portraitIdentity ? `已选出镜人像资料：${portraitIdentity}。所有 generationPlan.characterAppearance、voice、人物动作、神态和构图必须与该资料一致；不得根据商品、目标受众或营销场景另行推断或修改已提供的人像资料。` : ""}
