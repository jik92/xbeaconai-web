...
function UploadField({ field, value, onChange }: { field: FieldSpec; value: string; onChange: (value:string)=>void }) {
  ...
  return <label className="upload-zone">
    <input id={field.id} className="sr-only" type="file" accept={`${field.kind}/*`} onChange={(e)=>onChange(e.target.files?.[0]?.name ?? "")} />
...
export function ModulePage({ config }: { config: ModuleConfig }) {
  ...
  return <div className="module-page">
    ...
    <div className="workspace-grid"><section className="work-card">
      ...
      {currentStep<2?<div className="form-stack">{hydrated?visibleFields.map(field=><BusinessField key={field.id} field={field} value={values[field.id]??""} onChange={value=>setValue(field.id,value)} invalid={Boolean(submitted&&field.required&&!values[field.id])}/>):<><div className="form-skeleton"/><div className="form-skeleton"/><div className="form-skeleton wide"/></>}</div>:<div className="review-panel"><div className="review-head"><span><Check size={18}/></span><div><h3>确认创作配置</h3><p>检查以下信息，确认后任务将进入异步生成队列。</p></div></div><dl>{config.fields.map(field=><div key={field.id}><dt>{field.label}</dt><dd>{values[field.id]||"未设置"}</dd></div>)}</dl><button type="button" className="edit-config" onClick={()=>setCurrentStep(0)}>返回修改内容</button></div>}
      ...
      <div className="submit-bar"><div><b>{currentStep===2?`预计消耗 ${config.cost} 创作点`:`第 ${currentStep+1} 步，共 3 步`}</b><small>{hydrated?"已自动保存草稿":"正在恢复草稿…"} · {config.duration}</small></div><div className="wizard-actions">{currentStep>0&&<button type="button" className="secondary" onClick={back}><ArrowLeft/>上一步</button>}{currentStep<2?<button type="button" disabled={!hydrated} onClick={next}>下一步<ArrowRight/></button>:<button disabled={running||!hydrated} onClick={submit}>{running?<LoaderCircle className="animate-spin"/>:<WandSparkles/>}{running?"正在创作…":config.action}</button>}</div></div>
