...
const moduleRoutes=modules.map(config=>createRoute({getParentRoute:()=>rootRoute,path:config.path,component:()=> !isModuleOpen(config.id)?<ComingSoonPage config={config}/>:config.id==="video-remix"?<RemixProject/>:config.id==="ai-generate"?<AiGeneratePage/>:<ModulePage config={config}/> }));
