...
export function RemixProject() {
  ...
  const smartModify = () => {
    setPrompt((value) => `${value}\n\nAI 优化：强化前三秒冲突，用更口语化的表达突出限时处理理由。`);
    setNotice("已完成智能修改");
  };
  const merge = () => {
    if (job?.status === "succeeded") {
      setNotice("完整二创结果已生成，可预览与导出");
    } else {
      setNotice("任务仍在后台生成，完成后会自动更新");
    }
  };
    ...
    <div className="remix-project">
      ...
      {historyOpen && (
        <div className="safe-note">
          <History />
          <span>
            <b>{job?.title ?? "暂无当前项目任务"}</b>
            <small>
              {job ? `${job.stage} · ${job.progress}% · ${job.overallExecutionMode}` : "完成上传并解析后将显示项目记录"}
            </small>
          </span>
        </div>
      )}
      {notice && (
        <div className="safe-note">
          ...
          <span>
            <b>{notice}</b>
            <small>
              {job ? `任务 ${job.id.slice(0, 8)} · ${job.stage} ${job.progress}%` : `${APP_CONFIG.projectName}工作台`}
            </small>
          </span>
        </div>
      )}
      ...
      {stage === 0 && (
        <main className="setup-stage">
          <section>
            <h2>上传爆款参考</h2>
            <p>上传一条已验证的爆款视频，AI 将提取人物、商品、场景与分镜结构。</p>
            <label className="large-drop">
              <input
                id="remix-source"
                type="file"
                accept="video/*"
              ...
              <b>{uploading ? "正在上传…" : source ? source.split(":").slice(2).join(":") : "上传爆款视频"}</b>
              <span>{source ? "已安全上传，可重新选择" : "MP4/MOV，最大 500MB"}</span>
            </label>
            <div className="engine-panel">
              ...
              {!videoModels.length && (
                <small className="field-error">Seedance 尚未完成真实基线验证，暂不能提交生成。</small>
              )}
            </div>
          </section>
          <section>
            ...
            {selectedPortrait && (
              <div className="selected-portrait">
                ...
                <div>
                  <small>已从人像库带入</small>
                  <b>{selectedPortrait.profession}</b>
                  <span>{selectedPortrait.name}</span>
                </div>
                <Check />
              </div>
            )}
            <label>
              ...
              <textarea
                ...
                placeholder="描述商品卖点、目标人群和希望调整的表达风格…"
              />
            </label>
          </section>
        </main>
      ...
      {stage === 1 && (
        <main className="analysis-stage">
          ...
          <h2>{parsing ? "正在理解视频内容" : "准备进行 AI 解析"}</h2>
          <p>解析人物、商品、场景、口播文案和镜头边界。</p>
          {parsing && (
            <div className="analysis-progress">
              <span style={{ width: `${job?.progress ?? 10}%` }} />
              <b>{job?.stage ?? "正在提交解析任务…"}</b>
            </div>
          )}
          <button disabled={parsing} onClick={() => void parse()}>
            {parsing ? <LoaderCircle className="animate-spin" /> : <WandSparkles />}
            {parsing ? "解析中" : "开始 AI 解析"}
          </button>
        </main>
      )}

      {stage === 2 && (
        <main className="proof-stage">
          <aside className="proof-assets">
            ...
            <p className="project-name">古叔的着色巴拿马草帽男夏季大头围新款船夫帽</p>
            <label>
              商品 <em>*</em>
              <button onClick={() => setNotice("已从商品库选择演示商品")}>商品库</button>
            </label>
          ...
          <section className="proof-editor">
            <div className="source-card">
              <div className="source-thumb">
                <Video />
              </div>
              <div>
                <b>{source ? source.split(":").slice(2).join(":") : "13428656243498662.mp4"}</b>
                <span>已解析 · {job?.overallExecutionMode ?? "mock"}</span>
              </div>
              <button onClick={() => void parse()}>
                <RefreshCw />
              </button>
            </div>
            ...
            <div className="editor-toolbar">
              <label>
                对比版本{" "}
                <button className={`toggle ${compare ? "active" : ""}`} onClick={() => setCompare((value) => !value)} />
              </label>
              <div>
                ...
                <button onClick={smartModify}>
                  <WandSparkles />
                  智能修改
                </button>
                <button className="orange" onClick={() => setNotice("已切换为沉稳男声·云舟")}>
                  <Mic2 />
                  换口播
                </button>
              </div>
            ...
            <div className="editor-bottom">
              <div>
                ...
                <button onClick={() => void navigator.clipboard.writeText(prompt).then(() => setNotice("脚本已复制"))}>
                  <Copy />
                  复制脚本
                </button>
              </div>
      ...
      {stage === 3 && (
        <main className="storyboard-stage">
          ...
          <div className="shot-grid">
            {shotCopy.map((copy, index) => (
              <article key={copy}>
                <div className="shot-preview">
                  <Video />
                  <span>00:0{index + 3}</span>
                </div>
                <div>
                  <b>分镜 {String(index + 1).padStart(2, "0")}</b>
                  <p>{copy}</p>
                  <button
                    onClick={() => {
                      const replacement = window.prompt("编辑分镜口播", copy);
                      if (replacement) setNotice(`分镜 ${index + 1} 已更新`);
                    }}
                  >
                    编辑分镜
                  </button>
      ...
      {stage === 4 && (
        <main className="merge-stage">
          <div className="merge-preview">
            ...
            <button onClick={() => setNotice(result?.summary ?? "正在准备合成预览")}>预览合成效果</button>
          </div>
          <section>
            <span>最后一步 · {job?.overallExecutionMode ?? "mock"}</span>
            <h2>确认合并成片</h2>
            <p>{result?.summary ?? "4 个分镜已校对完成，将按照确认后的提示词、人物和口播合成为完整视频。"}</p>
            <dl>
              <div>
                <dt>画面比例</dt>
                <dd>9:16 竖屏</dd>
              ...
              <div>
                <dt>任务进度</dt>
                <dd>{job?.progress ?? 0}%</dd>
              </div>
            </dl>
            <button className="merge-button" onClick={merge}>
              <WandSparkles />
              {job?.status === "succeeded" ? "查看生成结果" : "等待合并成片"}
            </button>
            <div className="merge-actions">
              <button onClick={() => setNotice("已打开原片与成片对比")}>对比原片</button>
              <button onClick={() => setNotice("成片预览已就绪，可使用播放器控制")}>预览成片</button>
              <button onClick={downloadResult}>
                <Download />
                导出视频
              </button>
              <button onClick={() => setStage(2)}>再次改写</button>
            </div>
          </section>
        </main>
      )}
      ...
      {stage !== 2 && (
        <footer className="project-footer">
          {stage > 0 ? (
            <button onClick={() => setStage((value) => value - 1)}>
              <ArrowLeft />
              上一步
            </button>
          ) : (
