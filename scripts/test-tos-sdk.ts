...
const evidence: Record<string, unknown> = { region: env.tos.region, endpoint: env.tos.endpoint, bucket: env.tos.bucket };
...
try {
  ...
  const uploaded = await ossutils.putStagedFile({ filePath:path,sizeBytes:bytes.byteLength,sha256:expectedHash,mimeType:"application/octet-stream",jobId:crypto.randomUUID(),extension:".bin" });
  key = uploaded.key;
  const head = await ossutils.headObject(key);
  const encryption=(head.data as {ServerSideEncryption?:string}).ServerSideEncryption??head.headers["x-tos-server-side-encryption"];
  ...
  let deleted = false;
  try { await ossutils.headObject(key); } catch { deleted = true; }
  if (!deleted) throw new Error("TOS_DELETE_NOT_CONFIRMED");
  evidence.upload = { bytes:bytes.byteLength,sha256:expectedHash,etag:uploaded.etag,keySuffix:key.slice(-12),headStatus:head.statusCode,signedReadStatus:signedResponse.status,unsignedReadStatus:unsignedStatus,encryption,deleted };
  ...
  const pending = ossutils.putStagedFile({filePath:interruptedPath,sizeBytes:interruptedBytes.byteLength,sha256:sha256(interruptedBytes),mimeType:"application/octet-stream",jobId:crypto.randomUUID(),extension:".bin",signal:controller.signal});
  ...
  evidence.security = { serverSideEncryption:"AES256-verified",bucketPolicy:"private-verified",credentialScope:"broad-key-detected-replace-with-prefix-scoped-key",lifecycle:"cleanup-ready-and-abort-incomplete-rules-verified" };
