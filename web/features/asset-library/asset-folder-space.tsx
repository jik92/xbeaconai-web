...
export function AssetFolderSpace({ folders, selectedFolderId, loading, onSelect }: AssetFolderSpaceProps) {
  ...
  return (
    <aside className="flex h-full min-h-0 flex-col overflow-hidden bg-surface" aria-label="文件夹空间管理">
      <header className="flex h-8 flex-none items-center justify-between px-1">
        <b className="type-label text-ink">我的文件夹</b>
        <Button
          className="size-8"
          size="icon-sm"
          variant="ghost"
      ...
      <nav className="min-h-0 flex-1 overflow-y-auto py-1" aria-label="素材文件夹">
        ...
        {loading && <p className="px-2 py-3 type-helper text-muted">正在加载文件夹…</p>}
      </nav>
      <footer className="flex-none border-t border-line px-2 py-2">
        <span className="block type-helper text-muted">当前空间</span>
        <b className="mt-0.5 block truncate type-badge text-ink">
