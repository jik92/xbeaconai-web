...
export function ProviderFeatureGate({ moduleId, children }: { moduleId: ModuleId; children: ReactNode }) {
  ...
  if (query.isLoading)
    return (
      <main className="flex h-[calc(100vh-56px)] items-center justify-center bg-surface type-body text-muted">
  ...
  if (query.isError || !availability?.enabled)
    return (
      <main className="flex h-[calc(100vh-56px)] items-center justify-center bg-surface p-4">
        <div className="flex max-w-lg items-center gap-3 rounded-md border border-line bg-muted/20 px-4 py-3 type-body text-muted">
