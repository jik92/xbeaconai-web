...
页面不再维护一套“类似 AI 创作”的独立结构，而是把 `AiCreationComposer` 中的交互壳抽为共享 Composer。
AI 创作与素材理解必须使用同一套输入卡片、附件入口、参数触发器与弹层、提交按钮、任务表格、空状态和结果抽屉。
两个业务页面只注入标题、文案、可见参数、素材规则、提交函数和结果操作，确保后续交互修改不会再次分叉。
...
“添加参考素材”继续使用 AI 创作卡片左侧的同一个附件入口，而不是底部额外参数按钮。素材理解不显示创作类型、
画幅、清晰度、生成数量、时长、种子、参考模式、手动确认和星点报价控件。页面标题仍为“素材理解”，输入提示、
任务区标题和结果内容使用素材理解业务文案。
...
- 共享 Composer 的 AI 创作模式回归测试，证明现有生成参数、提交和任务交互未被改变。
- 共享 Composer 的素材理解模式测试，证明 DOM 结构与交互组件共用，同时仅显示模型、思考深度和附件入口。
