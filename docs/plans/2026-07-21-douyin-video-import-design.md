...
| `page.add_init_script("stealth.min.js")` | 下载器去除 `navigator.webdriver` 标记，并使用 `ignoreDefaultArgs: ["--enable-automation"]`；不伪造登录态、不绕过验证码。 |
...
- 本地排查可通过 `DOUYIN_BROWSER_HEADLESS=false` 打开临时 Chromium，并用 `DOUYIN_BROWSER_DEBUG_PAUSE_MS` 留出人工观察/登录时间；生产保持无头模式。
